...
def 기타성별구매(chart=True):
    ...
    # 비밀 성별 데이터 필터링
    male_data = order_df[order_df['성별'] == 'OTHER']
    ...
    if not chart:
        ...
        return top_10_products_male.to_csv(save_folder + "/기타성별구매.csv", index=False, encoding='utf-8')

    # 시각화
    plt.figure(figsize=(12, 8))

    # `hue=None`을 명시적으로 설정하여 경고를 해결
    ax = sns.barplot(x='수량', y='상품명', data=top_10_products_male, palette='spring', hue='상품명')

    # 제목, 레이블 설정
    plt.title('비밀 성별 상위 10개 구매 상품', fontsize=16)
    plt.xlabel('구매 수량', fontsize=12)
    plt.ylabel('상품명', fontsize=12)

    # x축 눈금을 짝수로 설정 (0, 2, 4, 6 등)
    ax.set_xticks(range(0, int(top_10_products_male['수량'].max()) + 2, 2))  # 짝수 눈금
