...
def 월별회원가입24(chart=True):
    ...
    df1 = df1[df1['회원가입일'].dt.year == 2024]
    ...
    if not chart:
        ...
        return monthly_signup.to_csv(save_folder + "/월별회원가입24.csv", encoding='utf-8')

    # 월만 추출 (연도-월에서 월만 추출)
    monthly_signup.index = monthly_signup.index.month

    # 그라데이션 색상 사용
    colors = plt.cm.Purples(monthly_signup.values / monthly_signup.max())  # 색상 맵
    ...
    # 차트 제목 및 레이블 설정
    plt.title('2024년 월 별 회원 가입 수', fontsize=16)
