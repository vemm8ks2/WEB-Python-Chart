...
def 남성구매(chart=True):
    ...
    # 남성 데이터 필터링
    male_data = order_df[order_df['성별'] == 'MALE']
    ...
    if not chart:
        ...
        return top_10_products_male.to_csv(save_folder + "/남성구매.csv", index=False, encoding='utf-8')

    # 시각화
    plt.figure(figsize=(12, 8))

    # `hue=None`을 명시적으로 설정하여 경고를 해결
    ax = sns.barplot(x='수량', y='상품명', data=top_10_products_male, palette='Blues_d', hue='상품명')

    # 제목, 레이블 설정
    plt.title('남성 상위 10개 구매 상품', fontsize=16)
