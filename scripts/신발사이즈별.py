...
def 신발사이즈별(chart=True):
    ...
    # 신발 사이즈 별이므로 옷 사이즈는 drop해준다
    shoes = order_items.groupby('사이즈')['수량'].sum().reset_index().drop(order_items.index[3:7])

    # 모든 신발 수량 더하기
    shoes_total = shoes.set_index('사이즈')['수량'].sum()

    # 사이즈를 인덱스로 변경
    shoes = shoes.set_index('사이즈')

    # 백분율 구하기
    shoes_total_size = ((shoes / shoes_total) * 100)
    
    if not chart:
        shoes_total_size = shoes_total_size.rename(columns=({'수량':'판매율'}))
        save_folder = os.getenv("REPORT_PREPROCESS_PWD")
        return shoes_total_size.to_csv(save_folder + "/신발사이즈별.csv", encoding='utf-8')
    ...
    # 데이터의 길이에 맞게 색상 수 조정
    colors_to_use = new_pastel_colors[:len(shoes_total_size)]

    # 파이 차트로 신발 사이즈별 판매율 시각화
    plt.figure(figsize=(8, 8))
    shoes_total_size.plot(kind='pie', autopct='%1.1f%%', startangle=90, colors=colors_to_use, subplots=True)
    plt.title('신발 사이즈별 판매율')
    plt.ylabel('')  # y축 라벨 제거
